...
std::vector<float> torus(float inner_radius, float outer_radius, int slices, int stacks) {
    ...
    for (int i = 0; i < slices; i++) {
        ...
        for (int j = 0; j < stacks; j++) {
            ...
            normals.push_back((outer_radius + cos(phi)) * cos(theta)); // x1
            normals.push_back(sin(phi)); // y1
            normals.push_back((outer_radius + cos(phi)) * sin(theta)); // z1
            ...
            normals.push_back((outer_radius + cos(nextPhi)) * cos(theta)); // x2
            normals.push_back(sin(nextPhi)); // y2
            normals.push_back((outer_radius + cos(nextPhi)) * sin(theta)); // z2
            ...
            normals.push_back((outer_radius + cos(phi)) * cos(nextTheta)); // x3
            normals.push_back(sin(phi)); // y3
            normals.push_back((outer_radius + cos(phi)) * sin(nextTheta)); // z3
            ...
            normals.push_back((outer_radius + cos(nextPhi)) * cos(nextTheta)); // x4
            normals.push_back(sin(nextPhi)); // y4
            normals.push_back((outer_radius + cos(nextPhi)) * sin(nextTheta)); // z4
